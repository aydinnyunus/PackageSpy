...
func DownloadAllPyPIPackagesSearch(search string) {
	client := &http.Client{}
	req, err := http.NewRequest("GET", "https://pypi.org/search/?q="+search, nil)
	if err != nil {
		log.Fatal(err)
	}
	req.Header.Set("authority", "pypi.org")
					...
					if attr.Key == "class" && attr.Val == "package-snippet" {
						...
						for _, attr := range token.Attr {
							if attr.Key == "href" {
								fmt.Println("Href:", "https://pypi.org"+attr.Val)
								downloadUrl := getPyPIDownloadUrl(attr.Val)
								downloadPyPIPackage(downloadUrl, search)
								utils.RunGitleaks("pypi_packages/" + search)
